...
def find_path_vector( lam, n, m , which_mod,flag=0):

    lam_key = vector(QQ,[which_mod]+lam.list())
    if store_path.exists(lam_key) and flag == 0:
        ...
        for w_so in W_so.W:
            ...
            for v in lowest_module_V:
                ...
                if remove_matches_vec(at_lambda_sp_plus_so,n,m) == 0:
                    typical_lambda_so = at_lambda_sp_plus_so[:n]
                    typical_lambda_sp = at_lambda_sp_plus_so[-m:]
    ...
    if store_path.exists(lam_key) and flag == 1:
        print("新结果是:")
        zhengli(results_V)
        print("旧结果是:")
        zhengli(store_path.get_group(lam_key))
        if_se_new = input("是否更新?")
        if if_se_new == "yes":
            store_path.remove_group(lam_key)
...
#                    break
            ...
            while True:
                try:
                    ...
                    print("输入无效，请输入一个整数。")
            print(f"which_mod: {which_mod}")
            results_V = find_path_vector( atypical_lambda_sp_plus_so, n, m,which_mod,1 )
