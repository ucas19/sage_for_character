...
# This file was *autogenerated* from the file B_2_function.sage
...
W = WeylGroup(['B', _sage_const_2 ], prefix='w')
print(f"✅ W(B₂) 群：{W}")
...
Z2 = AbelianGroup([_sage_const_2 , _sage_const_2 ], names='e')
e1, e2 = Z2.gens()
print(f"✅ (Z₂)² = {Z2}")
print(f"生成元: {e1}, {e2}")
print(f"所有元素: {list(Z2)}")

S2 = SymmetricGroup(_sage_const_2 )
s = S2.gen(_sage_const_0 )  # (1,2)
print(f"✅ S₂ = {S2}")
print(f"元素: {list(S2)}")
...
def twist_action(sigma, epsilon):
    ...
    if sigma == S2.identity():
        ...
        # 交换：σ = (1,2) ⇒ 新顺序: (ε2, ε1)
        return Z2((eps_tuple[_sage_const_1 ], eps_tuple[_sage_const_0 ]))
...
"""
def b2_to_g(w):
    e1 = vector(ZZ, [_sage_const_1 , _sage_const_0 ])
    e2 = vector(ZZ, [_sage_const_0 , _sage_const_1 ])
    w_mat = w.to_matrix()
    v1 = w_mat * e1
    v2 = w_mat * e2

    # 处理 v1 = w(e1)
    if v1[_sage_const_0 ] != _sage_const_0 :
        sigma_1 = _sage_const_1 
        eps1 = _sage_const_0  if v1[_sage_const_0 ] == _sage_const_1  else _sage_const_1 
    else:
        sigma_1 = _sage_const_2 
        eps2 = _sage_const_0  if v1[_sage_const_1 ] == _sage_const_1  else _sage_const_1 

    # 处理 v2 = w(e2)
    if v2[_sage_const_0 ] != _sage_const_0 :
        sigma_2 = _sage_const_1 
        eps1 = _sage_const_0  if v2[_sage_const_0 ] == _sage_const_1  else _sage_const_1 
    else:
        sigma_2 = _sage_const_2 
        eps2 = _sage_const_0  if v2[_sage_const_1 ] == _sage_const_1  else _sage_const_1 
...
def g_to_b2(g_elem):
    ...
    # 符号矩阵 D
    D = diagonal_matrix([_sage_const_1  if e == _sage_const_0  else -_sage_const_1  for e in eps_list])
    # 排列矩阵 P
    if sigma == S2.identity():
        P = matrix([[_sage_const_1 , _sage_const_0 ], [_sage_const_0 , _sage_const_1 ]])
    else:
        P = matrix([[_sage_const_0 , _sage_const_1 ], [_sage_const_1 , _sage_const_0 ]])
    full_mat = D * P
    # 在 W 中查找对应元素
    for w in W:
