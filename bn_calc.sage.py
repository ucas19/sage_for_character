...
_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_20 = Integer(20); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_4 = Integer(4); _sage_const_5 = Integer(5); _sage_const_6 = Integer(6); _sage_const_7 = Integer(7); _sage_const_8 = Integer(8); _sage_const_14 = Integer(14); _sage_const_13 = Integer(13); _sage_const_12 = Integer(12); _sage_const_9 = Integer(9); _sage_const_10 = Integer(10); _sage_const_11 = Integer(11)
            ...
            if removed_count == _sage_const_0 :
                ...
                if results_ten==[]:
                    ...
                    for i in range(len(results_ten)):
                        print(f"{i+_sage_const_1 }: {results_ten[i]}")
...
#    Dic_P_mu_tensor_V_after_Pr = Counter(P_mu_tensor_V_after_Pr)
    ...
    lambda_judge_hash, lambda_judge = judge_mu_in_P(at_lambda_sp_plus_so,n,m,_sage_const_20 )
    cant_judge = P_mu_tensor_V_after_Pr[:]
    if flag == _sage_const_1 :
...
def find_path_vector( lam, n, m , which_mod,flag=_sage_const_0 ):

    lam_key = vector(QQ,[which_mod]+lam.list())
    if store_path.exists(lam_key) and flag == _sage_const_0 :
    ...
    for w_sp in W_sp.W:
        for w_so in W_so.W:
            ...
            for v in lowest_module_V:
                ...
                P_vectors = []
                if remove_matches_vec(at_lambda_sp_plus_so,n,m) == _sage_const_0 :
                    typical_lambda_so = at_lambda_sp_plus_so[:n]
                    typical_lambda_sp = at_lambda_sp_plus_so[-m:]
                    P_vectors = test_K_L(n,m,typical_lambda_sp,typical_lambda_so,_sage_const_1 )
                else:
    ...
    if store_path.exists(lam_key) and flag == _sage_const_1 :
        print("新结果是:")
        zhengli(results_V)
        print("旧结果是:")
...
def auto_calc_long_time(store, at_lambda_sp_plus_so,n,m, sheaf):
    ...
    #首先找到那些容易计算的向量
    wheather_end = _sage_const_0 
    """
    补充，这里可以拓展使用的总的投射模
    """
    for i in range(_sage_const_3 ):
        which_mod = i+_sage_const_1 

        if wheather_end ==_sage_const_1 :
            break
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if select_case==_sage_const_13 :
            ...
            if sub_select_case == _sage_const_4 :
                store.list_keys()

            elif sub_select_case == _sage_const_3 :
                ...
                lowest_module = Lowest_Module(n,m)
                lowest_weight = which_one_lowest(weight_set,lowest_module.basis_plus)
                if not len(lowest_weight) == _sage_const_1 :
                    print("特征标权集有误！")
                    continue
                else:
                    print(f"即将储存特征标:{lowest_weight[_sage_const_0 ]}")
                    try:
                        store.add_group(lowest_weight[_sage_const_0 ], weight_set)
                    except ValueError as e:
                        print(e)  # Key vector ... already exists
        ...
        if select_case==_sage_const_12 :
            ...
            if len(weight_set)==_sage_const_0 :
                continue
                ...
                for v,k in weight_set_count.items():
                    ...
                    for i in range(len(v)):
                        if v[i]> _sage_const_0 :
                            str_s.append("\\fracc{"+str(abs(v[i])*_sage_const_2 )+"}")
                        else:
                            str_s.append("-\\fracc{"+str(abs(v[i])*_sage_const_2 )+"}")
                    ...
                    results = results[:-_sage_const_1 ] + '|' 
                    for i in range(m):
                        results = results + str_s[n+i] + ','
                    if k > _sage_const_1 :
                        print( str(k)+"M_{"+results[:-_sage_const_1 ] +"}")
                    else:
                        print( "M_{"+results[:-_sage_const_1 ] +"}")

            else: 
                num_tem_dict = {}
                for i in range(len(weight_set[_sage_const_0 ])):
                    num_tem = abs(weight_set[_sage_const_0 ][i])
                    if num_tem not in num_tem_dict:
                        ...
                        if char_c == '':
                            ...
                            if char_c[_sage_const_0 ] == '+':
                                char_c = char_c[_sage_const_1 :]

                            num_tem_dict[num_tem] = char_c 

                            if len(char_c)==_sage_const_1 :
                                num_tem_dict[-num_tem] = '-'+char_c
                            elif len(char_c)==_sage_const_2  and char_c[_sage_const_0 ] == '-':
                                num_tem_dict[-num_tem] =  char_c[_sage_const_1 :]
                            elif len(char_c)==_sage_const_3  and char_c[_sage_const_1 ] == '+':
                                num_tem_dict[-num_tem] = '-'+char_c[_sage_const_0 ]+'-'+char_c[_sage_const_2 ]
                            elif len(char_c)==_sage_const_3  and char_c[_sage_const_1 ] == '-':
                                num_tem_dict[-num_tem] = '-'+char_c[_sage_const_0 ]+'+'+char_c[_sage_const_2 ]
                            elif len(char_c)==_sage_const_4  and char_c[_sage_const_0 ] == '-' and char_c[_sage_const_2 ] == '+':
                                num_tem_dict[-num_tem] = char_c[_sage_const_1 ]+'-'+char_c[_sage_const_3 ]
                            elif len(char_c)==_sage_const_4  and char_c[_sage_const_0 ] == '-' and char_c[_sage_const_2 ] == '-':
                                num_tem_dict[-num_tem] = char_c[_sage_const_1 ]+'+'+char_c[_sage_const_3 ]
...
#            print(f"---------{removed_count}")
            ...
            while True:
                try:
                    ...
                    print("输入无效，请输入一个整数。")
            if sub_select_case == _sage_const_2 :
                user_input = input("请输入文档的名字:")# 将输入转换为有理数列表并创建向量
                with open('test//'+user_input+'.txt', 'r') as f:
                    lines = f.readlines()
...
#                    break
        ...
        elif select_case == _sage_const_11 :
            ...
            while True:
                try:
                    ...
                    print("输入无效，请输入一个整数。")
            print(f"which_mod: {which_mod}")
            results_V = find_path_vector( atypical_lambda_sp_plus_so, n, m,which_mod,_sage_const_1  )
