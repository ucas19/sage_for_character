...
_sage_const_1 = Integer(1); _sage_const_1en6 = RealNumber('1e-6'); _sage_const_50 = Integer(50); _sage_const_7 = Integer(7); _sage_const_10 = Integer(10); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_4 = Integer(4); _sage_const_0 = Integer(0); _sage_const_5 = Integer(5); _sage_const_9 = Integer(9)
...
def is_nonnegative_integer_combination_sage(A, A_list):
    ...
    # 系数不全为零：至少一个系数 >= 1
    p.add_constraint(sum(c[j] for j in range(k)) >= _sage_const_1 )
...
# 更稳健的版本，处理数值精度问题
def is_nonnegative_integer_combination_sage_robust(A, A_list, tolerance=_sage_const_1en6 ):
    """
    稳健版本，处理数值精度问题
    """
    p = MixedIntegerLinearProgram(maximization=False, solver="GLPK")
...
def testsssss():
    # 创建 SageMath 向量
    print("测试1: 可以表示的情况")
    A1 = vector(ZZ, [_sage_const_7 , _sage_const_10 ])  # ZZ 表示整数环
    A_list1 = [
        vector(ZZ, [_sage_const_1 , _sage_const_2 ]),
        vector(ZZ, [_sage_const_3 , _sage_const_4 ])
    ]
    test_sage_combination(A1, A_list1)
    
    print("测试2: 不能表示的情况")
    A2 = vector(ZZ, [_sage_const_1 , _sage_const_1 ])
    A_list2 = [
        vector(ZZ, [_sage_const_2 , _sage_const_0 ]),
        vector(ZZ, [_sage_const_0 , _sage_const_2 ])
    ]
    test_sage_combination(A2, A_list2)
    
    print("测试3: 零向量的特殊情况")
    A3 = vector(ZZ, [_sage_const_0 , _sage_const_0 ])
    A_list3 = [
        vector(ZZ, [_sage_const_1 , _sage_const_0 ]),
        vector(ZZ, [_sage_const_0 , _sage_const_1 ])
    ]
    test_sage_combination(A3, A_list3)
    
    print("测试4: 需要多个系数的情况")
    A4 = vector(ZZ, [_sage_const_5 , _sage_const_7 , _sage_const_9 ])
    A_list4 = [
        vector(ZZ, [_sage_const_1 , _sage_const_1 , _sage_const_1 ]),
        vector(ZZ, [_sage_const_1 , _sage_const_2 , _sage_const_3 ]),
        vector(ZZ, [_sage_const_2 , _sage_const_2 , _sage_const_2 ])
    ]
    test_sage_combination(A4, A_list4)
    
    print("测试5: 有理数向量测试")
    A5 = vector(QQ, [_sage_const_5 /_sage_const_2 , _sage_const_7 /_sage_const_2 ])  # QQ 表示有理数域
    A_list5 = [
        vector(QQ, [_sage_const_1 , _sage_const_2 ]),
        vector(QQ, [_sage_const_2 , _sage_const_1 ])
    ]
...
if __name__ == "__main__":
    ...
    for i, vec in enumerate(vectors, _sage_const_1 ):
        print(f"向量 {i}: {vec} (类型: {type(vec)})")
    
    # 示例：对向量进行操作
    if len(vectors) >= _sage_const_2 :
        print(f"\n向量运算示例:")
        print(f"向量1 + 向量2 = {vectors[_sage_const_0 ] + vectors[_sage_const_1 ]}")
        print(f"向量1 · 向量2 = {vectors[_sage_const_0 ].dot_product(vectors[_sage_const_1 ])}")
